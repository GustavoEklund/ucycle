...
class CreateUserTest extends TestCase
{
     ...
     * @throws ORMException
     */
    public function test_assert_create_user_with_already_used_email_throws_exception(): void
    {
        $user_repository = $this->createMock(UserRepository::class);
        $user_repository
            ->expects(self::once())
            ->method('findBy')
            ->with(...[['email' => 'any_email@example.com']])
            ->willReturn([(new User)->setEmail('any_email@example.com')]);

        $this->expectException(RuntimeException::class);
        $this->expectExceptionMessage('Este email já está sendo usado.');
